...
if(isset($_POST["sub"])) {
    ...
    if ($pwd != $pwds) {
        if ($lang == 'hu') {
    header("location: ../tregisztracio") ;
        exit();
}
else if ($lang == 'en') {
    header("location: ../en/tregisztracio") ;
        exit();
}
else if ($lang == 'rs') {
    header("location: ../rs/tregisztracio") ;
        exit();
}
    }
...
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($lang == 'hu') {
    header("location: ../tregisztracio?error=emailerror") ;
        exit();
}
else if ($lang == 'en') {
    header("location: ../en/tregisztracio?error=emailerror") ;
        exit();
}
else if ($lang == 'rs') {
    header("location: ../rs/tregisztracio?error=emailerror") ;
        exit();
}
  }


        if (uidLetezikTulaj($conn, $name, $email, $lang) !== false) {

if ($lang == 'hu') {
    header("location: ../tregisztracio?error=uidExists") ;
        exit();
}
else if ($lang == 'en') {
    header("location: ../en/tregisztracio?error=uidExists") ;
        exit();
}
else if ($lang == 'rs') {
    header("location: ../rs/tregisztracio?error=uidExists") ;
        exit();
}
        }
...
<i style="font-size: 18px;">Ako se prozor za potvrdu identiteta ne otvori, pokušajte ponovo ili kontaktirajte našu korisničku podršku na našoj veb stranici ili na loadeat.support@gmail.com ili loadeat@gmail.com! Ako ne izvršite autentifikaciju u određenom roku, naš sistem će izbrisati vaš nalog iz baze podataka. Želimo vam prijatno vreme.</i>
 ';
            ...
            $mail->send();
            createUserTulaj($conn, $id, $name, $email, $pwd, $ip, $verification_code, $lang);
}
...
{
     header("location: ../tregisztracio?error=problem") ;
        exit();
